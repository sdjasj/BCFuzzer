...
def extract_blob_hashes(transactions):
    ...
    for raw in transactions:
        ...
        try:
            ...
            # blob tx: [chainId, nonce, maxPriorityFee, maxFee, gas, to, value,
            #           data, accessList, blobVersionedHashes, maxFeePerBlobGas]
...
for i in range(ROUNDS):
    ...
    try:
        fcu_method = "engine_forkchoiceUpdatedV3" if API_VER == "v3" else "engine_forkchoiceUpdatedV1"
        fcu = rpc(fcu_method,
                  [{"headBlockHash": head,
                    "safeBlockHash": head,
        ...
        if MODE == "payload":
            ...
            get_method = "engine_getPayloadV3" if API_VER == "v3" else "engine_getPayloadV1"
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    if pid is None:
                        continue
                    env = rpc(get_method, [pid])
                    exec_payload = env["executionPayload"] if API_VER == "v3" else env
                    block_hash = exec_payload["blockHash"]
                    n_txs = len(exec_payload["transactions"])
                    vhashes = extract_blob_hashes(exec_payload["transactions"])
                    if API_VER == "v3":
