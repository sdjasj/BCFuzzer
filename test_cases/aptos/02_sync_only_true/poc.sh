...
SWARM_DIR=$(mktemp -d /tmp/poc-sync.XXXXXX)
...
lines = [l for l in lines if l != '  sync_only: true']  # 防御:清残留
...
lines.insert(end, '  sync_only: true')
open(path, 'w').write("\n".join(lines) + "\n")
d = yaml.safe_load(open(path))
assert d['consensus']['sync_only'] is True, "配置修改失败"
print("配置已修改: consensus.sync_only = true")
PYEOF
