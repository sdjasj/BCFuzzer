...
SWARM_DIR=$(mktemp -d /tmp/poc-rt0.XXXXXX)
...
echo "==> [4/6] SIGKILL 节点0,在 consensus 段末尾插入 round_initial_timeout_ms: 0"
...
lines = [l for l in lines if l != '  round_initial_timeout_ms: 0']  # 防御:清残留
idx = next(i for i, l in enumerate(lines) if re.match(r'^consensus:', l))
end = next(i for i in range(idx + 1, len(lines)) if re.match(r'^\S', lines[i]))  # consensus 段末尾
lines.insert(end, '  round_initial_timeout_ms: 0')
open(path, 'w').write("\n".join(lines) + "\n")
d = yaml.safe_load(open(path))
assert d['consensus']['round_initial_timeout_ms'] == 0, "配置修改失败"
print("配置已修改: consensus.round_initial_timeout_ms = 0")
PYEOF
...
TIMEOUTS=$(grep -c "Local timeout" "$SWARM_DIR/0/log" 2>/dev/null || echo 0)
LAST_ROUND=$(grep "Local timeout" "$SWARM_DIR/0/log" 2>/dev/null | tail -1 | grep -o '"round":[0-9]*' | head -1)
echo ">>> node0 超时次数: $TIMEOUTS, 最近轮次: ${LAST_ROUND:-无}"
if [ "$FROZEN" = "1" ] && [ "$TIMEOUTS" -gt 10 ]; then
    echo ">>> 复现成功: 两节点均存活但 ledger 冻结,轮次无限推进(TC 循环),全网共识永久停滞 (inter-node 影响确认)"
else
...
echo "==> 证据: 节点0日志 $SWARM_DIR/0/log (grep 'Local timeout');两节点 ledger 冻结"
